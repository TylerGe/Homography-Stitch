...
function [xp, yp, value] = harris(input_image, sigma,thd, r)
    ...
    % GENERATE GAUSSIAN FILTER OF SIZE 6*SIGMA (¡À 3SIGMA) AND OF MINIMUM SIZE 1x1
...
function [hh, inliers,points_1,points_2] = ransacfithomography(ref_P, dst_P, npoints, threshold);
...
function [H]=homographyget(x_init, y_init, npoints)
    ABB=[];
    for i=1:npoints
        x=x_init(:,i);
        y=y_init(:,i);
    ...
    n=norm(h);
    h=h/n;
    H=10000*[h(1:3,1)';h(4:6,1)';h(7:9,1)']/6;
end
